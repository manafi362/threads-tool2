...
(function () {
  ...
  var accent = current.dataset.color || "#0f766e";
  var preset = current.dataset.position || "bottom-right";
  var displayName = current.dataset.name || "サポートチャット";
  var welcome = current.dataset.welcome || "こんにちは。サイトについて知りたいことを聞いてください。";
  var x = Number(current.dataset.x || 24);
  var y = Number(current.dataset.y || 24);
  var sessionId = "";
  ...
  style.textContent = [
    ...
    ".launcher{width:60px;height:60px;border:none;border-radius:999px;background:" + accent + ";color:#fff;box-shadow:0 18px 42px rgba(15,23,42,.28);cursor:pointer;font-size:13px;font-weight:700}",
    ...
    ".header{padding:16px;background:" + accent + ";color:#fff;font-weight:700}",
    ...
    ".user{background:" + accent + ";color:#fff;align-self:flex-end}",
  ...
  launcher.setAttribute("aria-label", displayName);
  ...
  function applyPosition() {
    ...
    wrap.style.top = "";
    wrap.style.bottom = "";
    if (preset === "custom") { wrap.style.left = x + "px"; wrap.style.top = y + "px"; }
    else if (preset === "bottom-left") { wrap.style.left = x + "px"; wrap.style.bottom = y + "px"; }
    else if (preset === "top-left") { wrap.style.left = x + "px"; wrap.style.top = y + "px"; }
    else if (preset === "top-right") { wrap.style.right = x + "px"; wrap.style.top = y + "px"; }
    else { wrap.style.right = x + "px"; wrap.style.bottom = y + "px"; }
  }
  ...
  composer.addEventListener("submit", async function (event) {
    ...
    try {
      ...
      addMessage("assistant", data.answer || "その質問にはまだうまく答えられませんでした。");
    } catch (error) {
      addMessage("assistant", "送信に失敗しました。時間をおいてもう一度お試しください。");
    } finally {
      send.disabled = false;
    }
  });
